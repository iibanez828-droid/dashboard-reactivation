...
def _make_page_template(logo_bytes=None, eh5000_bytes=None):
    """Return an onPage callback + (topMargin, bottomMargin) for SimpleDocTemplate."""
    from reportlab.lib.pagesizes import letter
    from reportlab.lib import colors as rl_colors
    from reportlab.platypus import Image as RLImage

    PAGE_W, PAGE_H = letter
    _ORANGE = rl_colors.HexColor("#FF6B00")
    _GREY   = rl_colors.HexColor("#666666")
    _LGREY  = rl_colors.HexColor("#DDDDDD")

    # Header dimensions
    HDR_H    = 52   # total header height (pt)
    EH_W     = 110  # EH5000 image width
    EH_H     = 46   # EH5000 image height
    ...
    def _on_page(canvas, doc):
        ...
        if logo_bytes:
            try:
                logo_img = RLImage(BytesIO(logo_bytes), width=LOGO_W, height=LOGO_H)
                logo_img.drawOn(canvas, 22, PAGE_H - HDR_H + 10)
            except Exception:
                canvas.setFillColor(_ORANGE)
                canvas.setFont("Helvetica-Bold", 12)
                canvas.drawString(22, PAGE_H - HDR_H + 18, "LANDCROS")
        else:
            canvas.setFillColor(_ORANGE)
            canvas.setFont("Helvetica-Bold", 12)
            canvas.drawString(22, PAGE_H - HDR_H + 18, "LANDCROS")
        ...
        canvas.setFillColor(rl_colors.HexColor("#AAAAAA"))
        canvas.drawCentredString(PAGE_W / 2, PAGE_H - 33,
                                 "Landcros Engineering")

        # ── EH5000 image — right ─────────────────────────────────
        if eh5000_bytes:
            try:
                eh_img = RLImage(BytesIO(eh5000_bytes), width=EH_W, height=EH_H)
                eh_img.drawOn(canvas, PAGE_W - EH_W - 18, PAGE_H - HDR_H + 3)
            except Exception:
                pass   # silently skip if image cannot be rendered

        # ── Light grey rule under header ─────────────────────────
        canvas.setStrokeColor(_LGREY)
        canvas.setLineWidth(0.6)
        canvas.line(22, PAGE_H - HDR_H - 2, PAGE_W - 22, PAGE_H - HDR_H - 2)

        # ── Footer ───────────────────────────────────────────────
        canvas.setStrokeColor(_LGREY)
        canvas.setLineWidth(0.5)
        canvas.line(22, 28, PAGE_W - 22, 28)
        canvas.setFont("Helvetica", 6.5)
        canvas.setFillColor(rl_colors.HexColor("#999999"))
        canvas.drawString(22, 18,
            "Landcros Fleet Reactivation Analysis  |  Data: Data base Reactivation.xlsx")
        canvas.drawRightString(PAGE_W - 22, 18, f"Page {doc.page}")
