...
class AppConfig:
    ...
    page_title: str = "Landcros — Fleet Reactivation"
    chm_rate: float = 40.86
    currency: str = "USD"
...
REQUIRED_COLUMNS_BY_SHEET: dict[str, set[str]] = {
    "Structural": {
        "DT", "Hours", "Weighted criteria", "Total Labour",
    ...
    "Component parts impact": {
        "Category Item", "Component", "Total components required",
    },
    "Inventory total": {
        "Category Item", "QTY", "Total price",
    },
...
def normalize_columns(dataframe: pd.DataFrame) -> pd.DataFrame:
    """Return a copy with normalized string column names."""
    df = dataframe.copy()
    df.columns = [col.strip() if isinstance(col, str) else col for col in df.columns]
    return df


def validate_columns(sheet_name: str, dataframe: pd.DataFrame, required_columns: Iterable[str]) -> None:
    """Fail fast when an expected Excel schema changes."""
...
def numeric_series(dataframe: pd.DataFrame, column: str, default: float = 0.0) -> pd.Series:
...
def load_data(excel_path: Path):
    """Load, normalize, validate and cache all Excel sheets."""
    if not excel_path.exists():
        raise FileNotFoundError(
            f"Excel file not found: {excel_path}. Put the Excel file in the same folder as this .py file."
        )
    ...
    loaded = {
        sheet_name: normalize_columns(pd.read_excel(excel_path, sheet_name=sheet_name))
        for sheet_name in sheet_names
    }

    for sheet_name, required_columns in REQUIRED_COLUMNS_BY_SHEET.items():
        validate_columns(sheet_name, loaded[sheet_name], required_columns)
    ...
    return (
        ...
        loaded["Component $"],
        loaded["Kits"],
        loaded["Cerrejon inventory impact"],
        loaded["Component parts impact"],
        loaded["Inventory total"],
    )

LOGO_B64 = load_logo_b64()
try:
    structural, rules, labour_sht, comp_costs, kits_sht, cerrejon_impact, component_impact, inventory_total = load_data(EXCEL_PATH)
...
#  CONSTANTS
# ─────────────────────────────────────────────────────────────────
CHM_RATE = CONFIG.chm_rate   # USD/hr — from Rules & Rate column D
...
def _category_summary(df: pd.DataFrame, qty_col: str, value_col: str) -> pd.DataFrame:
    tmp = df.copy()

    if "Category Item" not in tmp.columns:
        return pd.DataFrame(columns=["Category Item", qty_col, value_col])

    tmp[qty_col] = numeric_series(tmp, qty_col)
...
def _render_category_cards(df: pd.DataFrame, title: str) -> None:
...
KIT_COLS = sorted(
    [c for c in labour_sht.columns if is_kit_col(c) and c in structural.columns],
    key=kit_number,
)
KIT_LABELS = [kit_label(c) for c in KIT_COLS]
...
# -----------------------------------------------------------------
def safe_component_value(row_label: str, comp_name: str) -> float:
...
def component_total_cost(comp_name: str) -> float:
    """Component cost from Component $ sheet."""
    lh   = safe_component_value("Labour hours", comp_name)
    lc   = safe_component_value("Labour cost", comp_name)
    mech = safe_component_value("Mechanized & Rebuild", comp_name)
    pts  = safe_component_value("parts", comp_name)
    chr_ = safe_component_value("Chrome tube & rod", comp_name)
    return (lh * lc) + mech + pts + chr_

COMP_TOTAL_COST = {c: component_total_cost(c) for c in set(FLAG_COL_TO_COMP.values())}

def apply_dynamic_component_costs(dataframe: pd.DataFrame) -> pd.DataFrame:
