...
def _fig_to_pdf_element(fig, width=520, height=285):
    ...
    try:
        ...
        return Paragraph(
            "Chart image could not be exported. Install the package 'kaleido' to include Plotly charts in PDF reports. "
...
def _add_fleet_overview_section(story, styles, report_df, figs):
    ...
    # Fleet Cost Detail table intentionally excluded from the PDF report.
...
def _add_cost_analysis_section(story, styles, report_df, figs):
    ...
    for label, values in susp_summary.items():
        table_data.append([
            label,
            _format_usd(values["front_cost"]),
            f'{values["front_count"]:,.0f}',
            _format_usd(values["rear_cost"]),
            f'{values["rear_count"]:,.0f}',
            _format_usd(values["total_cost"]),
...
def _add_kit_analysis_section(story, styles, report_df, figs):
    ...
    for kit_col, kit_label in zip(KIT_COLS, KIT_LABELS):
        if kit_col in report_df.columns:
            ...
            if qty > 0:
                kit_totals.append([kit_label, f"{qty:,.0f}"])
...
def _add_inventory_section(story, styles):
    ...
    if required_col and category_col and dt_col:
        ...
        story.append(Spacer(1, 10))
        data = [["Inventory Category", "Required Parts"]] + [[r[category_col], f"{r[required_col]:,.0f}"] for _, r in inv_summary.iterrows()]
        story.append(_pdf_table(data[:25], col_widths=[300, 120]))
    else:
        story.append(Paragraph("Inventory columns were not found for the report summary.", styles["BodyText"]))
...
def _add_part_list_section(story, styles):
    ...
    story.append(Paragraph(
        "Kits Inventory Table is intentionally excluded from this PDF report, according to the requested reporting scope.",
        styles["BodyText"],
    ))
