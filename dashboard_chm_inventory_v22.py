...
def _make_page_template(logo_bytes=None):
    ...
    _DARK   = rl_colors.HexColor("#1A1A1A")
    _GREY   = rl_colors.HexColor("#888888")
    ...
    def _on_page(canvas, doc):
        ...
        if logo_bytes:
            try:
                logo_img = RLImage(BytesIO(logo_bytes), width=95, height=26)
                logo_img.drawOn(canvas, 22, PAGE_H - 35)
            except Exception:
                canvas.setFillColor(_ORANGE)
                canvas.setFont("Helvetica-Bold", 11)
                canvas.drawString(22, PAGE_H - 25, "LANDCROS")
        else:
            canvas.setFillColor(_ORANGE)
            canvas.setFont("Helvetica-Bold", 11)
            canvas.drawString(22, PAGE_H - 25, "LANDCROS")
        ...
        canvas.drawRightString(PAGE_W - 22, PAGE_H - 20, "Fleet Reactivation — EH5000")
        canvas.setFont("Helvetica", 7)
        canvas.setFillColor(rl_colors.HexColor("#AAAAAA"))
        canvas.drawRightString(PAGE_W - 22, PAGE_H - 31, "Landcros Engineering")

        # Orange rule
        canvas.setStrokeColor(_ORANGE)
        canvas.setLineWidth(1.2)
        canvas.line(0, PAGE_H - 41, PAGE_W, PAGE_H - 41)
...
def build_dashboard_pdf(report_tab, include_all_tabs=False,
    ...
    on_page_cb, top_margin, bot_margin = _make_page_template(logo_bytes)
