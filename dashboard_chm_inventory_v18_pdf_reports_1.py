...
def load_data():
    ...
    kits_sht    = pd.read_excel(EXCEL_PATH, sheet_name="Kits")
...
# ─────────────────────────────────────────────────────────────────
#  PDF REPORT HELPERS  (reportlab — no external install needed)
# ─────────────────────────────────────────────────────────────────
...
def _rl_styles():
    ...
    s["title"] = ParagraphStyle("title", fontName="Helvetica-Bold", fontSize=18,
                                 textColor=_ORANGE, spaceAfter=4)
    ...
    s["section"] = ParagraphStyle("section", fontName="Helvetica-Bold", fontSize=9,
                                   textColor=_BLACK, spaceBefore=10, spaceAfter=2,
                                   textTransform="uppercase")
    s["body"] = ParagraphStyle("body", fontName="Helvetica", fontSize=8,
                                textColor=_BLACK, spaceAfter=4)
    s["small"] = ParagraphStyle("small", fontName="Helvetica", fontSize=7,
                                 textColor=_GREY, spaceAfter=2)
    s["cover_main"] = ParagraphStyle("cover_main", fontName="Helvetica-Bold", fontSize=26,
                                      textColor=_ORANGE, alignment=TA_CENTER, spaceAfter=8)
    s["cover_sub"] = ParagraphStyle("cover_sub", fontName="Helvetica", fontSize=14,
                                     textColor=_WHITE, alignment=TA_CENTER, spaceAfter=6)
    s["cover_info"] = ParagraphStyle("cover_info", fontName="Helvetica", fontSize=10,
                                      textColor=_GREY, alignment=TA_CENTER, spaceAfter=4)
    return s


def _header_footer(canvas_obj, doc):
    """Called on every page by reportlab."""
    canvas_obj.saveState()
    # ── Top bar ──
    canvas_obj.setFillColor(_BLACK)
    canvas_obj.rect(0, PAGE_H - 22 * mm, PAGE_W, 22 * mm, fill=1, stroke=0)
    canvas_obj.setFillColor(_ORANGE)
    canvas_obj.setFont("Helvetica-Bold", 12)
    canvas_obj.drawString(_MARGIN, PAGE_H - 13 * mm, "LANDCROS — Fleet Reactivation")
    canvas_obj.setFillColor(_GREY)
    canvas_obj.setFont("Helvetica", 7)
    canvas_obj.drawRightString(PAGE_W - _MARGIN, PAGE_H - 13 * mm,
                                date.today().strftime("%B %d, %Y"))
    # ── Orange rule under header ──
    canvas_obj.setStrokeColor(_ORANGE)
    canvas_obj.setLineWidth(1.5)
    canvas_obj.line(_MARGIN, PAGE_H - 23 * mm, PAGE_W - _MARGIN, PAGE_H - 23 * mm)
    # ── Footer ──
    canvas_obj.setStrokeColor(colors.HexColor("#EFEFEF"))
    canvas_obj.setLineWidth(0.5)
    canvas_obj.line(_MARGIN, 14 * mm, PAGE_W - _MARGIN, 14 * mm)
    canvas_obj.setFillColor(_GREY)
    canvas_obj.setFont("Helvetica", 6.5)
    canvas_obj.drawString(_MARGIN, 9 * mm,
                           "Landcros — Fleet Reactivation Analysis  |  Data: Data base Reactivation.xlsx")
    canvas_obj.drawRightString(PAGE_W - _MARGIN, 9 * mm,
                                f"Page {doc.page}")
    canvas_obj.restoreState()


def _cover_page_canvas(canvas_obj, doc):
    """Cover page: full black background, no header/footer bar."""
    canvas_obj.saveState()
    canvas_obj.setFillColor(_BLACK)
    canvas_obj.rect(0, 0, PAGE_W, PAGE_H, fill=1, stroke=0)
    # Orange accent strip
    canvas_obj.setFillColor(_ORANGE)
    canvas_obj.rect(0, PAGE_H * 0.42, PAGE_W, 3, fill=1, stroke=0)
    canvas_obj.restoreState()


def _kpi_table(kpis):
    """Return a reportlab Table for a row of KPI boxes.
    kpis = list of (label, value) tuples."""
    col_w = (PAGE_W - 2 * _MARGIN) / len(kpis)
    data = [[Paragraph(f"<font size='6' color='#888888'>{lbl.upper()}</font><br/>"
                        f"<font size='11' color='#1A1A1A'><b>{val}</b></font>",
                        ParagraphStyle("kpi", fontName="Helvetica", fontSize=9,
                                       leading=14, spaceAfter=0))
             for lbl, val in kpis]]
    tbl = Table(data, colWidths=[col_w] * len(kpis), rowHeights=[22 * mm])
...
def _df_table(df_slice, max_rows=40):
    """Return a reportlab Table from a DataFrame slice."""
    df_slice = df_slice.head(max_rows).reset_index(drop=True)
    cols = list(df_slice.columns)
    n = len(cols)
    usable_w = PAGE_W - 2 * _MARGIN
    col_w = usable_w / n

    header = [Paragraph(f"<b>{str(c)[:20]}</b>",
                         ParagraphStyle("th", fontName="Helvetica-Bold", fontSize=6.5,
                                        textColor=_WHITE, leading=8))
              for c in cols]
    rows = [header]
    for _, row in df_slice.iterrows():
        rows.append([
            Paragraph(str(row[c])[:22],
                       ParagraphStyle("td", fontName="Helvetica", fontSize=6.5,
                                      textColor=_BLACK, leading=8))
            for c in cols
        ])

    tbl = Table(rows, colWidths=[col_w] * n, repeatRows=1)
    tbl.setStyle(TableStyle([
...
def _section_elem(title, styles):
    return [
        Spacer(1, 4 * mm),
        Paragraph(title, styles["section"]),
        HRFlowable(width="100%", thickness=1, color=_ORANGE, spaceAfter=3),
    ]


# ── Main builders ──────────────────────────────────────────────────

def _build_tab_pdf(tab_name, df, df_cost_sorted, thresholds, kits_sht, cerrejon_impact, inventory_total):
    """Build a single-tab PDF and return bytes."""
    buf = BytesIO()
    ...
    story.append(Paragraph(tab_name, S["title"]))
    story.append(Paragraph(f"Report generated: {report_date}", S["subtitle"]))
    story.append(Spacer(1, 3 * mm))

    if tab_name == "Fleet Overview":
        ...
        story.append(_kpi_table([
            ("Total Fleet Cost",      f"${total_cost:,.0f}"),
            ("Avg Cost / Truck",       f"${avg_cost:,.0f}"),
            ("Avg Operating Hours",    f"{avg_hrs:,.0f} hrs"),
            ("Components to Replace",  str(total_flags)),
        ]))
        story += _section_elem("Fleet Cost Detail", S)
        tbl = df[["DT", "Hours", "Weighted criteria", "Total_Cost"]].copy().sort_values("Total_Cost", ascending=False)
        tbl.columns = ["DT", "Hours", "W. Criteria", "Total Cost (USD)"]
        tbl["DT"]   = tbl["DT"].astype(int)
        tbl["Hours"] = tbl["Hours"].map("{:,.0f}".format)
        tbl["W. Criteria"] = tbl["W. Criteria"].map("{:.3f}".format)
        tbl["Total Cost (USD)"] = tbl["Total Cost (USD)"].map("${:,.0f}".format)
        story.append(_df_table(tbl))

    elif tab_name == "Cost Analysis per Truck":
        story += _section_elem("Cost Breakdown by Truck", S)
        tbl = df[["DT", "Hours", "Total Labour", "Total cost per kit",
                   "Cost per Components", "Total_Cost"]].copy().sort_values("Total_Cost", ascending=False)
        tbl.columns = ["DT", "Hours", "Labour Hrs", "Kit Cost", "Comp Cost", "Total Cost"]
        tbl["DT"] = tbl["DT"].astype(int)
        for c in ["Kit Cost", "Comp Cost", "Total Cost"]:
            tbl[c] = tbl[c].map("${:,.0f}".format)
        story.append(_df_table(tbl))
        story += _section_elem("Active Component Thresholds", S)
        for cat, val in thresholds.items():
            story.append(Paragraph(f"  {cat}: {val*100:.0f}%", S["body"]))

    elif tab_name == "Kit Analysis":
        story += _section_elem("Kit Requirements per Truck", S)
        kit_cols_present = [c for c in KIT_COLS if c in df.columns]
        kit_totals = [(lbl, int((df[kc] >= 1).sum()))
                      for kc, lbl in zip(kit_cols_present, KIT_LABELS)]
        kit_totals.sort(key=lambda x: x[1], reverse=True)
        kit_data = [["Kit Name", "Trucks Required"]]
        for lbl, cnt in kit_totals:
            kit_data.append([str(lbl), str(cnt)])
        kt = Table(kit_data, colWidths=[(PAGE_W - 2*_MARGIN)*0.8, (PAGE_W - 2*_MARGIN)*0.2])
    ...
    elif tab_name == "Inventory Analysis":
        story += _section_elem("Cerrejón Inventory Impact", S)
        cerr = cerrejon_impact.copy().dropna(how="all")
        cerr.columns = [str(c).strip() for c in cerr.columns]
        story.append(_df_table(cerr.head(40)))

    elif tab_name == "Part List":
        story += _section_elem("Kits Inventory — Part List", S)
        inv = kits_sht.copy().dropna(how="all")
        inv.columns = [str(c).strip() for c in inv.columns]
        show_cols = [c for c in ["Kit", "Part Number", "Description", "QTY", "Total price"] if c in inv.columns]
        if not show_cols:
            show_cols = list(inv.columns[:5])
        story.append(_df_table(inv[show_cols].head(60)))

    elif tab_name == "Reactivation Gantt":
        story += _section_elem("Reactivation Scheduling Summary", S)
        story.append(Paragraph(
            "Trucks are ranked by Weighted Criteria (ascending). Lower score = better structural "
            "condition = higher reactivation priority.", S["body"]))
        story.append(Spacer(1, 3 * mm))
        tbl = df[["DT", "Hours", "Weighted criteria", "Total_Cost"]].copy().sort_values("Weighted criteria")
        tbl.columns = ["DT", "Hours", "Priority Score", "Est. Cost (USD)"]
        tbl["DT"]   = tbl["DT"].astype(int)
        tbl["Est. Cost (USD)"] = tbl["Est. Cost (USD)"].map("${:,.0f}".format)
        story.append(_df_table(tbl))
    else:
        story.append(Paragraph(f"No content defined for tab: {tab_name}", S["body"]))

    doc.build(story, onFirstPage=_header_footer, onLaterPages=_header_footer)
    return buf.getvalue()


def _build_full_pdf(df, df_cost_sorted, thresholds, kits_sht, cerrejon_impact, inventory_total):
    """Build the complete project PDF covering all tabs."""
    buf = BytesIO()
    report_date = date.today().strftime("%B %d, %Y")

    # We need two page templates: cover (black bg) and normal (branded header/footer).
    # Simplest approach: build cover separately, then append normal pages.
    from reportlab.platypus import BaseDocTemplate, Frame, PageTemplate

    def cover_tmpl(canvas_obj, doc):
        _cover_page_canvas(canvas_obj, doc)

    def normal_tmpl(canvas_obj, doc):
    ...
    cover_frame  = Frame(_MARGIN, _MARGIN, PAGE_W - 2*_MARGIN, PAGE_H - 2*_MARGIN,
                          id="cover")
    normal_frame = Frame(_MARGIN, 20*mm, PAGE_W - 2*_MARGIN,
                          PAGE_H - 26*mm - 20*mm, id="normal")

    doc.addPageTemplates([
        PageTemplate(id="Cover",  frames=[cover_frame],  onPage=cover_tmpl),
        PageTemplate(id="Normal", frames=[normal_frame], onPage=normal_tmpl),
    ])
    ...
    story.append(Paragraph("EH5000 FLEET REACTIVATION", S["cover_main"]))
    story.append(Paragraph("Full Project Report", S["cover_sub"]))
    story.append(Spacer(1, 6 * mm))
    story.append(Paragraph(f"Generated: {report_date}", S["cover_info"]))
    story.append(Paragraph(
        f"Active Trucks: {len(df)}  &nbsp;|&nbsp;  "
        f"Total Fleet Cost: ${df['Total_Cost'].sum():,.0f} USD",
        S["cover_info"]))

    # Switch to normal template for all content pages
    from reportlab.platypus import NextPageTemplate
    story.append(NextPageTemplate("Normal"))
    story.append(PageBreak())

    # ── Fleet Overview ──
    story.append(Paragraph("Fleet Overview", S["title"]))
    story.append(Spacer(1, 3 * mm))
    total_cost  = df["Total_Cost"].sum()
    avg_cost    = df["Total_Cost"].mean()
    avg_hrs     = df["Hours"].mean()
    total_flags = sum(
        int(df[f"_flag_{c}"].sum()) for c in FLAG_COL_TO_COMP.values()
        if f"_flag_{c}" in df.columns
    )
    story.append(_kpi_table([
        ("Total Fleet Cost",     f"${total_cost:,.0f}"),
        ("Avg Cost / Truck",      f"${avg_cost:,.0f}"),
        ("Avg Oper. Hours",       f"{avg_hrs:,.0f} hrs"),
        ("Components to Replace", str(total_flags)),
    ]))
    story += _section_elem("Fleet Cost Detail — All Active Trucks", S)
    tbl = df[["DT", "Hours", "Weighted criteria", "Total Labour",
              "Total cost per kit", "Cost per Components", "Total_Cost"]].copy().sort_values("Total_Cost", ascending=False)
    tbl.columns = ["DT", "Hours", "W. Criteria", "Labour Hrs", "Kit Cost", "Comp Cost", "Total Cost"]
    tbl["DT"] = tbl["DT"].astype(int)
    for c in ["Kit Cost", "Comp Cost", "Total Cost"]:
        tbl[c] = tbl[c].map("${:,.0f}".format)
    story.append(_df_table(tbl))
    ...
    story.append(Paragraph("Cost Analysis per Truck", S["title"]))
    story.append(Spacer(1, 3 * mm))
    thr_row = [(cat, f"{val*100:.0f}%") for cat, val in thresholds.items()]
    story.append(_kpi_table(thr_row))
    story += _section_elem("Component Flags per Truck", S)
    comp_flag_rows = []
    for _, row in df.iterrows():
        flag_count = sum(
            1 for c in FLAG_COL_TO_COMP.values()
            if f"_flag_{c}" in df.columns and int(row.get(f"_flag_{c}", 0)) == 1
        )
        comp_flag_rows.append({
            "DT": int(row["DT"]),
            "Hours": f"{row['Hours']:,.0f}",
            "Flagged Components": flag_count,
            "Total Cost (USD)": f"${row['Total_Cost']:,.0f}",
        })
    story.append(_df_table(pd.DataFrame(comp_flag_rows).sort_values("Flagged Components", ascending=False)))
    story.append(PageBreak())

    # ── Kit Analysis ──
    story.append(Paragraph("Kit Analysis", S["title"]))
    story.append(Spacer(1, 3 * mm))
    story += _section_elem("Kit Requirements Across Fleet", S)
    kit_cols_present = [c for c in KIT_COLS if c in df.columns]
    kit_totals = [(lbl, int((df[kc] >= 1).sum()))
                  for kc, lbl in zip(kit_cols_present, KIT_LABELS)]
    kit_totals.sort(key=lambda x: x[1], reverse=True)
    ...
    # ── Reactivation Gantt ──
    story.append(Paragraph("Reactivation Gantt", S["title"]))
    story.append(Spacer(1, 3 * mm))
    story += _section_elem("Planned Reactivation Priority", S)
    story.append(Paragraph(
        "Trucks ranked by Weighted Criteria (ascending). "
        "Lower score = better structural condition = higher priority.", S["body"]))
    story.append(Spacer(1, 3 * mm))
    gantt_tbl = df[["DT", "Hours", "Weighted criteria", "Total_Cost"]].copy().sort_values("Weighted criteria")
    gantt_tbl.columns = ["DT", "Hours", "Priority Score", "Est. Cost (USD)"]
    gantt_tbl["DT"] = gantt_tbl["DT"].astype(int)
    gantt_tbl["Est. Cost (USD)"] = gantt_tbl["Est. Cost (USD)"].map("${:,.0f}".format)
    story.append(_df_table(gantt_tbl))
    story.append(PageBreak())

    # ── Inventory Analysis ──
    story.append(Paragraph("Inventory Analysis", S["title"]))
    story.append(Spacer(1, 3 * mm))
    story += _section_elem("Cerrejón Inventory Impact", S)
    cerr = cerrejon_impact.copy().dropna(how="all")
    cerr.columns = [str(c).strip() for c in cerr.columns]
    story.append(_df_table(cerr.head(50)))
    story.append(PageBreak())

    # ── Part List ──
    story.append(Paragraph("Part List (Kits)", S["title"]))
    story.append(Spacer(1, 3 * mm))
    story += _section_elem("Kits Inventory — Part List", S)
    inv = kits_sht.copy().dropna(how="all")
    inv.columns = [str(c).strip() for c in inv.columns]
    ...
    if not show_cols:
        show_cols = list(inv.columns[:5])
    story.append(_df_table(inv[show_cols].head(80)))
...
with st.sidebar:
    ...
    # ── Report 1: Tab-specific PDF ──
    st.markdown(
        '<p style="font-size:0.77rem;color:#888;margin-bottom:8px;">'
        "Generate a PDF summary for a specific dashboard tab.</p>",
        unsafe_allow_html=True,
    )
    ...
    selected_pdf_tab = st.selectbox(
        "Select tab to export",
        options=pdf_tab_options,
        key="pdf_tab_selector",
    )

    if st.button("📥 Export Tab Report", use_container_width=True, key="btn_tab_pdf"):
        st.session_state["trigger_tab_pdf"] = selected_pdf_tab

    st.markdown("<div style='height:6px;'></div>", unsafe_allow_html=True)

    # ── Report 2: Full project PDF ──
    st.markdown(
        '<p style="font-size:0.77rem;color:#888;margin-bottom:8px;">'
        "Generate a full PDF report with all tabs and KPIs.</p>",
        unsafe_allow_html=True,
    )

    if st.button("📋 Export Full Project Report", use_container_width=True, key="btn_full_pdf"):
        st.session_state["trigger_full_pdf"] = True
...
SUSPENSION_GROUPS = {
    ...
    "Rear Suspensions": ["Rear strtus right", "Rear strut left"],
}
...
with tab_truck:
    ...
    suspension_components = [
        "Front strut right", "Rear strtus right", "Front strut left", "Rear strut left"
    ]
...
with tab_inventory_total:
    ...
    # ==========================================================
      ...
      # ==========================================================
    # USE FIRST COLUMN AS KIT NAME COLUMN
    # ==========================================================

    kit_column = inv_tbl.columns[0]
...
# ── Tab-specific PDF ──
if st.session_state.get("trigger_tab_pdf"):
    tab_name = st.session_state.pop("trigger_tab_pdf")
    with st.spinner(f"Generating PDF for '{tab_name}'…"):
        try:
            pdf_bytes = _build_tab_pdf(
                tab_name, df, df_cost_sorted, thresholds,
                kits_sht, cerrejon_impact, inventory_total
            )
            file_name = tab_name.lower().replace(" ", "_") + "_report.pdf"
            st.sidebar.download_button(
                label=f"⬇️ Download — {tab_name}",
                data=pdf_bytes,
                file_name=file_name,
            ...
            st.sidebar.success("✅ PDF listo — haz clic arriba para descargar.")
        except Exception as e:
            st.sidebar.error(f"Error generando PDF: {e}")

# ── Full project PDF ──
if st.session_state.get("trigger_full_pdf"):
    st.session_state.pop("trigger_full_pdf")
    with st.spinner("Generando reporte completo — un momento…"):
        try:
            pdf_bytes = _build_full_pdf(
                df, df_cost_sorted, thresholds,
                kits_sht, cerrejon_impact, inventory_total
            )
            st.sidebar.download_button(
                label="⬇️ Download — Full Project Report",
                data=pdf_bytes,
                file_name="landcros_fleet_full_report.pdf",
                mime="application/pdf",
                use_container_width=True,
                key="dl_full_pdf",
            )
            st.sidebar.success("✅ Reporte completo listo — haz clic arriba para descargar.")
        except Exception as e:
            st.sidebar.error(f"Error generando PDF: {e}")
