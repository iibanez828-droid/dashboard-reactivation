...
with tab_inventory:
    ...
    truck_part_totals = cerrejon_impact[active_truck_cols].apply(pd.to_numeric, errors="coerce").fillna(0).sum().reset_index()
    truck_part_totals.columns = ["Truck", "Required Parts"]
    truck_part_totals = truck_part_totals.sort_values("Required Parts", ascending=False)

    st.markdown('<div class="section-title">Cerrejon Inventory Impact — Required Parts by Truck</div>', unsafe_allow_html=True)
    st.plotly_chart(_bar_chart(truck_part_totals, "Truck", "Required Parts", "Total required parts by truck", "Required parts"), use_container_width=True, config={"displayModeBar": False})
