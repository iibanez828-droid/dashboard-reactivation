...
def build_severity_heatmap(report_df):
    sev_cols = [
        "High Arch Severity", "Nose Cone Severity", "Inside Web Plates Severity",
        "Hoist Plates Severity", "Top & Bottom flange Severity"
    ]
    if not all(col in report_df.columns for col in sev_cols):
        return None

    sev_data = report_df[["DT"] + sev_cols].set_index("DT")
    sev_data.columns = [
        "High Arch", "Nose Cone", "Web Plates", "Hoist Plates", "Top/Bot Flange"
    ]
...
def build_hours_vs_cost_fig(report_df):
    ...
    fig = go.Figure(go.Scatter(
        ...
        marker=dict(
            size=12,
            color=report_df.get("Weighted criteria", 0),
...
def build_gantt_fig(report_df, start_date=date.today(), overlap_days=10):
    ...
    fig = go.Figure(go.Bar(
        ...
        textfont=dict(size=10, family="Barlow Condensed", color="#FFFFFF"),
        hovertemplate=(
            "DT %{y}<br>Start: %{customdata[0]}<br>Finish: %{customdata[1]}<br>"
            "Labour: %{customdata[2]:,.0f} hrs<br>Total cost: $%{customdata[3]:,.0f}<extra></extra>"
        ),
        customdata=list(zip(
...
def build_truck_analysis_figures(report_df):
    ...
    truck_row = report_df[report_df["DT"].astype(int) == selected_dt].iloc[0]
    ...
    fig_susp = None
    susp_df = cdf[cdf["Component"].isin(["Front strut right", "Rear strtus right", "Front strut left", "Rear strut left"])].copy()
    if not susp_df.empty:
        fig_susp = go.Figure()
...
def _format_kits_table_data(dataframe, max_rows=30):
    ...
    for col in df.columns:
        ...
        if lower_col in {"category", "categoria", "category item"}:
            ...
            col_map[col] = "Total required Project"

    df = df.rename(columns=col_map)
    desired_columns = ["Category", "PN", "Description", "QTY per truck", "Total required Project"]
    ...
    for _, row in df.head(max_rows).iterrows():
        ...
        for col in selected_columns:
            value = row[col]
            if col == "QTY per truck":
                try:
                    rendered.append(f"{float(value):,.0f}")
                except Exception:
                    rendered.append(str(value))
            elif col == "Total required Project":
                try:
                    rendered.append(f"{float(value):,.0f}")
                except Exception:
                    rendered.append(str(value))
...
def _report_selected_truck_kit_rows(report_df):
    ...
    if not kit_rows:
        return []
    header = ["Kit", "Quantity", "Labour Hours", "Labour Cost (USD)", "Parts Cost (USD)", "Total Cost (USD)"]
...
def _report_inventory_kpi_table(report_df):
    ...
    selected_dt = selected_truck_cols[0] if selected_truck_cols else None
    if selected_dt is None:
        return [["Metric", "Value"], ["Selected truck", "N/A"]]

    inv = cerrejon_impact.copy()
    inv["_truck_qty"] = pd.to_numeric(inv[selected_dt], errors="coerce").fillna(0)
    inv["_truck_cost"] = inv["_truck_qty"] * pd.to_numeric(inv.get("Price 2026", 0), errors="coerce").fillna(0)
...
def _add_cost_analysis_section(story, styles, report_df, figs):
    ...
    story.append(_pdf_table([
        ...
        ["Hours", f"{truck_figs["truck_row"]["Hours"]:,.0f}"],
        ["Weighted Criteria", f"{truck_figs["truck_row"]["Weighted criteria"]:.3f}"],
    ], col_widths=[220, 220]))
    story.append(Spacer(1, 10))
...
def _add_kit_analysis_section(story, styles, report_df, figs):
    ...
    if selected_rows:
        ...
        story.append(Spacer(1, 6))
        story.append(_pdf_table(selected_rows, col_widths=[110, 60, 70, 80, 80, 80]))


def _add_gantt_section(story, styles, report_df):
    from reportlab.platypus import Paragraph, Spacer
    story.append(Paragraph("Reactivation Gantt", styles["Heading2"]))
    story.append(_fig_to_pdf_element(build_gantt_fig(report_df)))
    story.append(Spacer(1, 10))
...
def _add_inventory_section(story, styles):
    from reportlab.platypus import Paragraph, Spacer
    ...
    if required_col and category_col and dt_col:
        ...
        if selected_truck_cols:
            ...
            truck_tmp["_truck_cost"] = truck_tmp["_truck_qty"] * pd.to_numeric(truck_tmp.get("Price 2026", 0), errors="coerce").fillna(0)
...
def _add_part_list_section(story, styles):
    ...
    if len(kit_data) > 1:
        ...
        for page_index, start in enumerate(range(0, total_rows, page_size), start=1):
            ...
            # Prefer wider Category/Description columns but keep total width under page limits
            width_map = {
                "Category": 130,
                "PN": 90,
                "Description": 140,
                "QTY per truck": 50,
                "Total required Project": 70,
            }
            col_widths = [width_map.get(col, max(50, int(420 / max(len(header), 1)))) for col in header]
...
def build_dashboard_pdf(report_tab, include_all_tabs=False):
    ...
    section_builders = {
        ...
        "Inventory Analysis": lambda: _add_inventory_section(story, styles),
...
with tab_inventory_total:
    ...
    for col in ["QTY per truck", "Total required Project"]:
    ...
    with t2:
        total_qty = inv_tbl["QTY per truck"].sum() if "QTY per truck" in inv_tbl.columns else 0
        st.markdown(
            f'''
            <div class="kpi-card">
    ...
    with t3:
        total_req = inv_tbl["Total required Project"].sum() if "Total required Project" in inv_tbl.columns else 0
        st.markdown(
            f'''
            <div class="kpi-card">
                <div class="kpi-label">Total Required Project</div>
                <div class="kpi-value" style="font-size:1.45rem;">
                    {total_req:,.0f}
                </div>
            </div>
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "QTY per truck": st.column_config.NumberColumn(
                "QTY per truck",
                format="%.0f"
            ),
            "Total required Project": st.column_config.NumberColumn(
                "Total required Project",
                format="%.0f"
            ),
        },
    )
