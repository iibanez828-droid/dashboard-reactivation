...
def _add_part_list_section(story, styles):
    ...
    if len(kit_data) > 1:
        ...
        for page_index, start in enumerate(range(0, total_rows, page_size), start=1):
            ...
            # Prefer wider Category/Description columns to avoid overlap
            width_map = {
                "Category": 150,
                "PN": 110,
                "Description": 180,
                "QTY per truck": 80,
                "Total required Project": 120,
            }
            col_widths = [width_map.get(col, max(60, int(480 / max(len(header), 1)))) for col in header]
