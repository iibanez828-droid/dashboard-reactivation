...
def _pdf_header(pdf, title, subtitle=""):
    """Draw the branded header on a PDF page."""
    pdf.set_fill_color(26, 26, 26)
    pdf.rect(0, 0, 210, 28, "F")
    pdf.set_font("Helvetica", "B", 16)
    pdf.set_text_color(255, 107, 0)
    pdf.set_xy(10, 7)
    pdf.cell(0, 8, "LANDCROS — Fleet Reactivation", ln=False)
    if subtitle:
...
def _pdf_kpi_row(pdf, kpis):
    """Render a row of KPI boxes. kpis = list of (label, value) tuples."""
    ...
    for i, (label, value) in enumerate(kpis):
        ...
        pdf.rect(x, y0, col_w - 2, 18, "FD")
        pdf.set_xy(x + 2, y0 + 2)
        pdf.set_font("Helvetica", "B", 6)
        pdf.set_text_color(136, 136, 136)
        pdf.cell(col_w - 4, 4, str(label).upper(), ln=False)
        pdf.set_xy(x + 2, y0 + 7)
        pdf.set_font("Helvetica", "B", 10)
        pdf.set_text_color(26, 26, 26)
        pdf.cell(col_w - 4, 6, str(value), ln=False)
    pdf.set_xy(10, y0 + 22)
...
def _pdf_dataframe(pdf, df_slice, max_rows=30):
    """Render a simple table from a DataFrame slice."""
    df_slice = df_slice.head(max_rows).copy()
    cols = list(df_slice.columns)
    col_w = min(185 / len(cols), 40)
    # Header row
    pdf.set_fill_color(26, 26, 26)
    ...
    for i, (_, row) in enumerate(df_slice.iterrows()):
        if pdf.get_y() > 265:
            ...
            pdf.set_xy(10, 35)
        fill = i % 2 == 0
        pdf.set_fill_color(247, 247, 247) if fill else pdf.set_fill_color(255, 255, 255)
        pdf.set_text_color(26, 26, 26)
        for col in cols:
            val = str(row[col])[:18]
...
def _build_tab_pdf(tab_name, df, df_cost_sorted, thresholds, kits_sht, cerrejon_impact, inventory_total):
    ...
    _pdf_header(pdf, tab_name, f"Report generated: {report_date}")
    ...
    if tab_name == "Fleet Overview":
        ...
        _pdf_kpi_row(pdf, [
            ...
            ("Avg Hours",         f"{avg_hrs:,.0f} hrs"),
            ("Components to Replace", str(total_flags)),
        ])
        _pdf_section(pdf, "Fleet Cost Detail")
        tbl = df[["DT", "Hours", "Weighted criteria", "Total_Cost"]].copy().sort_values("Total_Cost")
        ...
        _pdf_dataframe(pdf, tbl)

    elif tab_name == "Cost Analysis per Truck":
        _pdf_section(pdf, "Cost Breakdown by Truck")
        ...
        _pdf_dataframe(pdf, tbl)
        _pdf_section(pdf, "Active Component Thresholds")
        pdf.set_font("Helvetica", "", 8)
        pdf.set_text_color(26, 26, 26)
        for cat, val in thresholds.items():
            pdf.cell(0, 5, f"  {cat}: {val*100:.0f}%", ln=True)
        pdf.ln(2)

    elif tab_name == "Kit Analysis":
        _pdf_section(pdf, "Kit Requirements per Truck")
        ...
        for lbl, cnt in kit_totals:
            bar_w = int((cnt / max(1, len(df))) * 80)
            pdf.set_fill_color(255, 107, 0)
            y = pdf.get_y()
            if y > 265:
                pdf.add_page(); pdf.set_xy(10, 35)
            pdf.cell(90, 5, str(lbl)[:45], ln=False)
            pdf.cell(15, 5, str(cnt), ln=False)
            pdf.set_fill_color(255, 107, 0)
            pdf.rect(115, pdf.get_y() + 1, max(bar_w, 1), 3, "F")
            pdf.ln(6)

    elif tab_name == "Inventory Analysis":
        _pdf_section(pdf, "Cerrejón Inventory Impact")
        cerr = cerrejon_impact.copy().dropna(how="all")
        cerr.columns = [str(c).strip() for c in cerr.columns]
        _pdf_dataframe(pdf, cerr.head(30))

    elif tab_name == "Part List":
        _pdf_section(pdf, "Kits Inventory — Part List")
        ...
        _pdf_dataframe(pdf, inv[show_cols].head(50))

    elif tab_name == "Reactivation Gantt":
        _pdf_section(pdf, "Gantt / Scheduling Summary")
        pdf.set_font("Helvetica", "", 9)
        pdf.set_text_color(80, 80, 80)
        pdf.multi_cell(0, 6,
            "The Reactivation Gantt tab provides an interactive Gantt chart that visualises "
            "the planned reactivation schedule per truck. For a static summary, refer to the "
            "Fleet Cost Detail table in the Fleet Overview tab.")
        pdf.ln(4)
        _pdf_section(pdf, "Fleet Schedule Summary")
        tbl = df[["DT", "Hours", "Weighted criteria", "Total_Cost"]].copy().sort_values("Weighted criteria", ascending=False)
        tbl.columns = ["DT", "Hours", "Priority (W. Criteria)", "Est. Cost (USD)"]
        tbl["DT"] = tbl["DT"].astype(int)
        tbl["Est. Cost (USD)"] = tbl["Est. Cost (USD)"].map("${:,.0f}".format)
        _pdf_dataframe(pdf, tbl)
    else:
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(0, 8, f"No content defined for tab: {tab_name}", ln=True)
...
def _build_full_pdf(df, df_cost_sorted, thresholds, kits_sht, cerrejon_impact, inventory_total):
    ...
    pdf.set_text_color(255, 107, 0)
    pdf.set_xy(0, 90)
    pdf.cell(210, 14, "EH5000 FLEET REACTIVATION", align="C", ln=True)
    pdf.set_font("Helvetica", "", 14)
    pdf.set_text_color(255, 255, 255)
    pdf.cell(210, 10, "Full Project Report", align="C", ln=True)
    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(170, 170, 170)
    pdf.set_xy(0, 130)
    pdf.cell(210, 7, f"Generated: {report_date}", align="C", ln=True)
    pdf.set_xy(0, 140)
    pdf.cell(210, 7, f"Active Trucks: {len(df)}  |  Total Fleet Cost: ${df['Total_Cost'].sum():,.0f} USD", align="C", ln=True)

    # ── Tab 1: Fleet Overview ──
    pdf.add_page()
    _pdf_header(pdf, "Fleet Overview", report_date)
    total_cost  = df["Total_Cost"].sum()
    avg_cost    = df["Total_Cost"].mean()
    avg_hrs     = df["Hours"].mean()
    total_flags = sum(
    ...
    _pdf_section(pdf, "Fleet Cost Detail — All Active Trucks")
    ...
    pdf.add_page()
    _pdf_header(pdf, "Cost Analysis per Truck", report_date)
    _pdf_section(pdf, "Active Component Thresholds")
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(26, 26, 26)
    thr_row = [(cat, f"{val*100:.0f}%") for cat, val in thresholds.items()]
    _pdf_kpi_row(pdf, thr_row)
    pdf.ln(6)
    _pdf_section(pdf, "Component Flags per Truck")
    comp_flag_rows = []
    ...
    _pdf_dataframe(pdf, pd.DataFrame(comp_flag_rows).sort_values("Flagged Components", ascending=False))

    # ── Tab 3: Kit Analysis ──
    pdf.add_page()
    _pdf_header(pdf, "Kit Analysis", report_date)
    _pdf_section(pdf, "Kit Requirements Across Fleet")
    kit_cols_present = [c for c in KIT_COLS if c in df.columns]
    kit_totals = [(lbl, int((df[kc] >= 1).sum()))
                  for kc, lbl in zip(kit_cols_present, KIT_LABELS)]
    kit_totals.sort(key=lambda x: x[1], reverse=True)
    ...
    for lbl, cnt in kit_totals:
        if pdf.get_y() > 265:
            pdf.add_page(); pdf.set_xy(10, 35)
        pdf.cell(100, 5, str(lbl)[:50], ln=False)
        pdf.cell(20, 5, f"{cnt} truck(s)", ln=True)
    pdf.ln(2)

    # ── Tab 4: Reactivation Gantt ──
    pdf.add_page()
    _pdf_header(pdf, "Reactivation Gantt", report_date)
    _pdf_section(pdf, "Planned Reactivation Priority")
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(80, 80, 80)
    pdf.multi_cell(0, 5,
        "Trucks are ranked by Weighted Criteria (ascending). Lower criteria = better structural "
        "condition = higher reactivation priority. Use this order to plan the maintenance schedule.")
    pdf.ln(3)
    gantt_tbl = df[["DT", "Hours", "Weighted criteria", "Total_Cost"]].copy().sort_values("Weighted criteria")
    gantt_tbl.columns = ["DT", "Hours", "Priority Score", "Est. Cost"]
    gantt_tbl["DT"]   = gantt_tbl["DT"].astype(int)
    gantt_tbl["Est. Cost"] = gantt_tbl["Est. Cost"].map("${:,.0f}".format)
    _pdf_dataframe(pdf, gantt_tbl)

    # ── Tab 5: Inventory Analysis ──
    pdf.add_page()
    _pdf_header(pdf, "Inventory Analysis", report_date)
    cerr = cerrejon_impact.copy().dropna(how="all")
    cerr.columns = [str(c).strip() for c in cerr.columns]
    _pdf_section(pdf, "Cerrejón Inventory Impact")
    _pdf_dataframe(pdf, cerr.head(40))

    # ── Tab 6: Part List ──
    pdf.add_page()
    _pdf_header(pdf, "Part List (Kits)", report_date)
    inv = kits_sht.copy().dropna(how="all")
    inv.columns = [str(c).strip() for c in inv.columns]
    _pdf_section(pdf, "Kits Inventory — Part List")
    show_cols = [c for c in ["Kit", "Part Number", "Description", "QTY", "Total price"] if c in inv.columns]
    if not show_cols:
        show_cols = list(inv.columns[:5])
    _pdf_dataframe(pdf, inv[show_cols].head(80))
...
def _pdf_footer(pdf):
    ...
    pdf.set_text_color(170, 170, 170)
    pdf.cell(0, 5, f"Landcros — Fleet Reactivation Analysis  |  {date.today().strftime('%Y-%m-%d')}  |  Data: Data base Reactivation.xlsx", align="C")
...
if st.session_state.get("trigger_tab_pdf"):
    ...
    with st.spinner(f"Generating PDF for '{tab_name}'…"):
        try:
            from fpdf import FPDF  # noqa: F401 — verify available
            ...
            if pdf_bytes:
                ...
                st.sidebar.success(f"✅ PDF ready — click above to download.")
            else:
                st.sidebar.error("PDF generation failed. Check fpdf2 installation.")
        except ImportError:
            st.sidebar.error(
...
if st.session_state.get("trigger_full_pdf"):
    ...
    with st.spinner("Generating full project PDF — this may take a moment…"):
        try:
            ...
            if pdf_bytes:
                ...
                st.sidebar.success("✅ Full report ready — click above to download.")
            else:
                st.sidebar.error("PDF generation failed. Check fpdf2 installation.")
        except ImportError:
            st.sidebar.error(
