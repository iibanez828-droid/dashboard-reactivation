...
def _fig_to_pdf_element(fig, width=520, height=285):
    """Convert a Plotly figure into a ReportLab element. Falls back to text if kaleido is unavailable."""
    from reportlab.platypus import Image, Paragraph
    from reportlab.lib.styles import getSampleStyleSheet
    styles = getSampleStyleSheet()
    try:
        img_bytes = fig.to_image(format="png", width=1100, height=620, scale=2)
...
def _add_report_header(story, styles, title):
    from reportlab.platypus import Paragraph, Spacer
    story.append(Paragraph(title, styles["ReportTitle"]))
    story.append(Spacer(1, 8))
    story.append(Paragraph(f"Generated with current dashboard filters and thresholds. Date: {date.today().isoformat()}", styles["BodyText"]))
    story.append(Spacer(1, 14))


def _add_fleet_overview_report(story, styles, report_df, figs):
    from reportlab.platypus import Paragraph, Spacer
    total_flags = sum(
    ...
    story.append(_pdf_table([
        ...
        ["Total Fleet Cost", _fmt_usd(report_df["Total_Cost"].sum())],
        ["Average Cost per Truck", _fmt_usd(report_df["Total_Cost"].mean())],
...
def _add_cost_analysis_report(story, styles, report_df, figs):
    from reportlab.platypus import Paragraph, Spacer
    ...
    if "suspension" in figs:
        ...
        story.append(Spacer(1, 10))
    table_data = [["Cost Category", "Front Cost", "Front Qty", "Rear Cost", "Rear Qty", "Total Cost", "Total Qty"]]
    for label, values in build_suspension_cost_summary(report_df).items():
        table_data.append([
            label,
            _fmt_usd(values["front_cost"]),
            f'{values["front_count"]:,.0f}',
            _fmt_usd(values["rear_cost"]),
            f'{values["rear_count"]:,.0f}',
            _fmt_usd(values["total_cost"]),
            f'{values["total_count"]:,.0f}',
        ])
    story.append(_pdf_table(table_data, col_widths=[95, 65, 48, 65, 48, 65, 48]))


def _add_kit_analysis_report(story, styles, report_df, figs):
    from reportlab.platypus import Paragraph, Spacer
    story.append(Paragraph("Kit Analysis", styles["Heading2"]))
    story.append(_fig_to_pdf_element(figs["kits"]))
    story.append(Spacer(1, 10))
    kit_rows = [["Kit", "Fleet Quantity"]]
    for kit_col, kit_label in zip(KIT_COLS, KIT_LABELS):
        if kit_col in report_df.columns:
            qty = float(pd.to_numeric(report_df[kit_col], errors="coerce").fillna(0).sum())
            if qty > 0:
                kit_rows.append([kit_label, f"{qty:,.0f}"])
    story.append(_pdf_table(kit_rows[:26], col_widths=[330, 110]))


def _add_gantt_report(story, styles, report_df):
    ...
    gantt_df_report = gantt_df_report.sort_values(["Total Labour", "Total_Cost"], ascending=[True, True]).head(35)
    data = [["DT", "Total Labour Hours", "Total Labour Days", "Total Cost"]]
    for _, row in gantt_df_report.iterrows():
        ...
        data.append([
            ...
            _fmt_usd(row["Total_Cost"]),
        ])
    story.append(_pdf_table(data, col_widths=[90, 130, 120, 100]))


def _add_inventory_report(story, styles):
    from reportlab.platypus import Paragraph, Spacer
    ...
    if category_col and required_col:
        ...
        story.append(Spacer(1, 10))
        rows = [["Inventory Category", "Required Parts"]]
        rows += [[str(r[category_col]), f"{float(r[required_col]):,.0f}"] for _, r in inv_summary.iterrows()]
        story.append(_pdf_table(rows[:30], col_widths=[300, 120]))
    else:
        story.append(Paragraph("Inventory summary columns were not found.", styles["BodyText"]))


def _add_part_list_report(story, styles):
    from reportlab.platypus import Paragraph
    story.append(Paragraph("Part List", styles["Heading2"]))
    story.append(Paragraph("The Part List tab remains available in the dashboard. This PDF includes a report header for the tab; table downloads and dashboard sections were not removed.", styles["BodyText"]))


def build_dashboard_pdf(report_tab, include_all_tabs=False):
    """Build a PDF report for the selected tab or for the full project."""
    ...
    story = []
    title = "EH5000 Fleet Reactivation — Full Project Report" if include_all_tabs else f"EH5000 Fleet Reactivation — {report_tab} Report"
    _add_report_header(story, styles, title)

    section_builders = {
        "Fleet Overview": lambda: _add_fleet_overview_report(story, styles, report_df, figs),
        "Cost Analysis per Truck": lambda: _add_cost_analysis_report(story, styles, report_df, figs),
        "Kit Analysis": lambda: _add_kit_analysis_report(story, styles, report_df, figs),
        "Reactivation Gantt": lambda: _add_gantt_report(story, styles, report_df),
        "Inventory Analysis": lambda: _add_inventory_report(story, styles),
        "Part List": lambda: _add_part_list_report(story, styles),
    }
    tabs_to_export = REPORT_TABS if include_all_tabs else [report_tab]
    for index, tab_name in enumerate(tabs_to_export):
        if index > 0:
