...
with tab_kit_analysis:
    ...
    selected_dt = st.selectbox(
        "Select Truck (DT)",
        options=sorted(active_truck_cols),
        key="kit_dt_selector"
    )

    truck_df = kits_df.copy()

    truck_df[selected_dt] = pd.to_numeric(
        truck_df[selected_dt],
        errors="coerce"
    ).fillna(0)

    truck_df = truck_df[truck_df[selected_dt] > 0]

    total_kit_types = int(truck_df.shape[0])
    total_quantity = float(truck_df[selected_dt].sum())

    total_labour_cost = float(
        pd.to_numeric(
            truck_df["Labour Cost"],
            errors="coerce"
        ).fillna(0).sum()
    )

    total_kit_cost = float(
        pd.to_numeric(
            truck_df["Total Cost (USD)"],
            errors="coerce"
        ).fillna(0).sum()
    )

    parts_cost = total_kit_cost - total_labour_cost

    operating_hours = 0

    if "Hours" in df_sorted_base.columns:

        hours_match = df_sorted_base[
            df_sorted_base["DT"].astype(str) == str(selected_dt)
        ]

        if not hours_match.empty:
            operating_hours = float(hours_match["Hours"].iloc[0])
    ...
    ">
        <div style="display:flex;justify-content:space-between;align-items:center;">
            ...
            <div>
                <div style="font-size:14px;color:#B0B0B0;letter-spacing:2px;">
                    TRUCK
                </div>
                <div style="font-size:54px;font-weight:900;color:#FF6B00;">
                    DT {selected_dt}
                </div>
            </div>
    ...
    with k1:
        st.metric("TOTAL KIT TYPES", int(total_kit_types))

    with k2:
        st.metric("TOTAL QUANTITY", f"{total_quantity:,.0f}")

    with k3:
        st.metric("TOTAL LABOUR COST", f"${total_labour_cost:,.0f}")

    with k4:
        st.metric("TOTAL PARTS COST", f"${parts_cost:,.0f}")

    with k5:
        st.metric("TOTAL KIT COST", f"${total_kit_cost:,.0f}")
