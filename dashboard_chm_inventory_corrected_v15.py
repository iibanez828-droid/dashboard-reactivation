...
def _fig_to_png_array(fig, width=1200, height=700):
    """Return a Plotly figure as a PNG array when Kaleido is available."""
    try:
        from PIL import Image
        img_bytes = fig.to_image(format="png", width=width, height=height, scale=2)
        return np.asarray(Image.open(BytesIO(img_bytes)))
    except Exception:
        return None


def _build_report_figures(report_df):
    figs = {}
    ...
    for comp_name in FLAG_COL_TO_COMP.values():
        ...
        if flag_col in report_df.columns:
            comp_counts[comp_name] = int(pd.to_numeric(report_df[flag_col], errors="coerce").fillna(0).sum())
    comp_series = pd.Series(comp_counts).sort_values(ascending=False).head(20)
    ...
    figs["components"].update_layout(
        ...
        margin=dict(l=155, r=30, t=50, b=40),
        paper_bgcolor="#FFFFFF",
        plot_bgcolor="#FFFFFF",
    ...
    if not susp_df.empty:
        ...
        figs["suspension"].add_trace(go.Bar(name="Front", x=susp_df["Cost Category"], y=susp_df["Front"]))
        figs["suspension"].add_trace(go.Bar(name="Rear", x=susp_df["Cost Category"], y=susp_df["Rear"]))
...
def _new_pdf_page(pdf, title, subtitle=None):
    import matplotlib.pyplot as plt
    fig = plt.figure(figsize=(11, 8.5))
    fig.patch.set_facecolor("white")
    ax = fig.add_axes([0, 0, 1, 1])
    ax.axis("off")
    ax.text(0.5, 0.94, title, ha="center", va="top", fontsize=18, fontweight="bold")
    if subtitle:
        ax.text(0.5, 0.90, subtitle, ha="center", va="top", fontsize=9, color="dimgray")
    return fig, ax


def _add_text_block(ax, lines, x=0.06, y=0.84, line_gap=0.035, fontsize=10):
    cursor = y
    for line in lines:
        ax.text(x, cursor, line, ha="left", va="top", fontsize=fontsize)
        cursor -= line_gap
    return cursor


def _add_plotly_image_or_note(ax, fig, x=0.06, y=0.08, w=0.88, h=0.48):
    arr = _fig_to_png_array(fig)
    if arr is not None:
        img_ax = ax.figure.add_axes([x, y, w, h])
        img_ax.imshow(arr)
        img_ax.axis("off")
    else:
        ax.text(
            x,
            y + h / 2,
            "Chart placeholder: install 'kaleido' in the Streamlit environment to render Plotly charts inside the PDF.",
            ha="left",
            va="center",
            fontsize=9,
            color="dimgray",
...
def _save_fleet_overview_pdf_page(pdf, report_df, figs):
    fig, ax = _new_pdf_page(
        pdf,
        "Fleet Overview",
        f"Generated with current dashboard filters and thresholds — {date.today().isoformat()}",
    )
    total_flags = sum(
        int(pd.to_numeric(report_df[f"_flag_{c}"], errors="coerce").fillna(0).sum())
        for c in FLAG_COL_TO_COMP.values()
        if f"_flag_{c}" in report_df.columns
    )
    _add_text_block(ax, [
        f"Active Trucks: {len(report_df):,.0f}",
        f"Total Fleet Cost: {_fmt_usd(report_df['Total_Cost'].sum())}",
        f"Average Cost per Truck: {_fmt_usd(report_df['Total_Cost'].mean())}",
        f"Average Operating Hours: {report_df['Hours'].mean():,.0f}",
        f"Components to Replace: {total_flags:,.0f}",
    ], y=0.84, fontsize=11)
    _add_plotly_image_or_note(ax, figs["fleet_cost"], y=0.12, h=0.55)
    pdf.savefig(fig, bbox_inches="tight")
    import matplotlib.pyplot as plt
    plt.close(fig)


def _save_cost_analysis_pdf_page(pdf, report_df, figs):
    fig, ax = _new_pdf_page(
        pdf,
        "Cost Analysis per Truck",
        f"Generated with current dashboard filters and thresholds — {date.today().isoformat()}",
    )
    lines = ["Suspension Repair Cost KPIs — Fleet Scope"]
    for label, values in build_suspension_cost_summary(report_df).items():
        lines.append(
            f"{label}: Front {_fmt_usd(values['front_cost'])} ({values['front_count']:,.0f} comp.) | "
            f"Rear {_fmt_usd(values['rear_cost'])} ({values['rear_count']:,.0f} comp.) | "
            f"Total {_fmt_usd(values['total_cost'])} ({values['total_count']:,.0f} comp.)"
        )
    _add_text_block(ax, lines, y=0.84, line_gap=0.04, fontsize=9)
    _add_plotly_image_or_note(ax, figs.get("suspension", figs["fleet_cost"]), y=0.10, h=0.48)
    pdf.savefig(fig, bbox_inches="tight")
    import matplotlib.pyplot as plt
    plt.close(fig)


def _save_kit_analysis_pdf_page(pdf, report_df, figs):
    fig, ax = _new_pdf_page(
        pdf,
        "Kit Analysis",
        f"Generated with current dashboard filters and thresholds — {date.today().isoformat()}",
    ...
    for kit_col, kit_label in zip(KIT_COLS, KIT_LABELS):
        if kit_col in report_df.columns:
            ...
            if qty > 0:
                kit_rows.append(f"{kit_label}: {qty:,.0f}")
    _add_text_block(ax, ["Fleet Kit Quantities"] + kit_rows[:12], y=0.84, line_gap=0.032, fontsize=9)
    _add_plotly_image_or_note(ax, figs["kits"], y=0.08, h=0.48)
    pdf.savefig(fig, bbox_inches="tight")
    import matplotlib.pyplot as plt
    plt.close(fig)


def _save_gantt_pdf_page(pdf, report_df):
    fig, ax = _new_pdf_page(
        pdf,
        "Reactivation Gantt",
        f"Generated with current dashboard filters and thresholds — {date.today().isoformat()}",
    )
    gantt_df_report = report_df[["DT", "Total Labour", "Total_Cost"]].copy()
    gantt_df_report = gantt_df_report.sort_values(["Total Labour", "Total_Cost"], ascending=[True, True]).head(22)
    lines = ["Selected truck sequence by Total Labour and Total Cost:"]
    for _, row in gantt_df_report.iterrows():
        labour_hours = _safe_number(row["Total Labour"])
        lines.append(
            f"DT {int(row['DT'])}: {labour_hours:,.0f} labour hours | {labour_hours / 24:.1f} days | {_fmt_usd(row['Total_Cost'])}"
        )
    _add_text_block(ax, lines, y=0.84, line_gap=0.032, fontsize=9)
    pdf.savefig(fig, bbox_inches="tight")
    import matplotlib.pyplot as plt
    plt.close(fig)


def _save_inventory_pdf_page(pdf):
    fig, ax = _new_pdf_page(
        pdf,
        "Inventory Analysis",
        f"Generated with current dashboard filters and thresholds — {date.today().isoformat()}",
    ...
    category_col = "Inventory category" if "Inventory category" in inv.columns else None
    required_col = "Required parts" if "Required parts" in inv.columns else None
    if required_col is None:
        numeric_cols = inv.select_dtypes(include=[np.number]).columns.tolist()
        required_col = numeric_cols[-1] if numeric_cols else None
    if category_col and required_col:
        ...
        top_lines = [f"{str(r[category_col])}: {float(r[required_col]):,.0f}" for _, r in inv_summary.head(10).iterrows()]
        _add_text_block(ax, ["Top Inventory Categories"] + top_lines, y=0.84, line_gap=0.035, fontsize=9)
        _add_plotly_image_or_note(ax, fig_inv, y=0.08, h=0.46)
    else:
        _add_text_block(ax, ["Inventory summary columns were not found."], y=0.84)
    pdf.savefig(fig, bbox_inches="tight")
    import matplotlib.pyplot as plt
    plt.close(fig)


def _save_part_list_pdf_page(pdf):
    fig, ax = _new_pdf_page(
        pdf,
        "Part List",
        f"Generated with current dashboard filters and thresholds — {date.today().isoformat()}",
    ...
    _add_text_block(ax, [
        "The Part List tab remains available in the dashboard.",
        "This PDF adds a report page without removing any existing dashboard section or table download.",
    ], y=0.84, fontsize=11)
    pdf.savefig(fig, bbox_inches="tight")
    import matplotlib.pyplot as plt
    plt.close(fig)
...
def build_dashboard_pdf(report_tab, include_all_tabs=False):
    """Build a dependency-light PDF report for the selected tab or for the full project."""
    from matplotlib.backends.backend_pdf import PdfPages

    buffer = BytesIO()
    report_df = df.copy()
    figs = _build_report_figures(report_df)

    page_builders = {
        "Fleet Overview": lambda pdf: _save_fleet_overview_pdf_page(pdf, report_df, figs),
        "Cost Analysis per Truck": lambda pdf: _save_cost_analysis_pdf_page(pdf, report_df, figs),
        "Kit Analysis": lambda pdf: _save_kit_analysis_pdf_page(pdf, report_df, figs),
        "Reactivation Gantt": lambda pdf: _save_gantt_pdf_page(pdf, report_df),
        "Inventory Analysis": lambda pdf: _save_inventory_pdf_page(pdf),
        "Part List": lambda pdf: _save_part_list_pdf_page(pdf),
    }
    tabs_to_export = REPORT_TABS if include_all_tabs else [report_tab]

    with PdfPages(buffer) as pdf:
        for tab_name in tabs_to_export:
            page_builders[tab_name](pdf)
...
with st.sidebar:
    ...
    st.caption("Generate PDF reports using the current thresholds and selected fleet filters.")
    ...
    report_tab_choice = st.selectbox(
        ...
        index=0,
        help=(
            "Streamlit does not expose the currently opened tab to Python, so select the tab you want to export. "
            "Use the full project button to include all tabs."
        ),
    )

    try:
        current_tab_pdf = build_dashboard_pdf(report_tab_choice, include_all_tabs=False)
        st.download_button(
            "Generate current tab PDF",
        ...
        st.warning(f"Current tab PDF could not be generated: {exc}")

    try:
        full_project_pdf = build_dashboard_pdf(report_tab_choice, include_all_tabs=True)
        st.download_button(
        ...
        st.warning(f"Full project PDF could not be generated: {exc}")
...
with tab_truck:
    ...
    if not susp_df.empty:
        ...
        # ── Fleet-scope suspension repair cost KPIs ──
