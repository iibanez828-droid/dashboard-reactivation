...
def _report_kpi_table(report_df):
    ...
    _cer_col = "Impact Cerrejon inventory"
    _rpt_cer_tot = pd.to_numeric(
        cerrejon_impact[_cer_col] if _cer_col in cerrejon_impact.columns
        else pd.Series(0, index=cerrejon_impact.index),
        errors="coerce"
    ).fillna(0).sum()
    _rpt_comp_tot = pd.to_numeric(
        component_impact[_cer_col] if _cer_col in component_impact.columns
        else pd.Series(0, index=component_impact.index),
        errors="coerce"
    ).fillna(0).sum()
